...
class Config:
    ...
    def from_toml(cls, filepath):
        ...
        for level in obj['levels']:
            flair_template_id = level.get('flair_template_id', None)
            if flair_template_id == '':
                flair_template_id = None
            # levels.append(Level(level['name'], level['points'], flair_template_id))
            levels.append((level['name'], level['points'], flair_template_id))
...
# TODO move to app or maybe cli module?
